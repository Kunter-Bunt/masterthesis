...
Gleichzeitig sorgt das kurze Sendeintervall aber auch für einem hohen Energieverbrauch auf Seiten der mobilen Einheit, eine Reduktion der Sendevorgänge sollte im Kontext der Bereichsotung angestrebt werden um den Energieverbrauch zu senken und die Batterielaufzeit der mobilen Einheit zu steigern.
...
Tabelle \ref{table:radarconsumption} zeigt den gemessenen Verbrauch der Implementierung für RADAR Tags jeweils in Arduino und C, mit unterschiedlich langen Sendeintervallen und mit und ohne manuellen \texttt{light\_sleep}.
...
\begin{table}[h]
	...
	\begin{tabular}{p{3cm}|p{2.2cm}|p{1.5cm}|p{2cm}|p{2cm}|p{2cm}}
		SDK & manueller \texttt{light\_sleep} & Sendeintervall in s & Spannung in V & $\varnothing$ Verbrauch in mA & $\varnothing$ Verbrauch in mW \\
		\hline
...
Die Implementierunges mit der ESP Open SDK und manuellem \texttt{light\_sleep} unterscheiden sich ausschließlich im Sendeintervall, dies verändert die projezierte Laufzeit jedoch stark.
...
Das Tag mit dem längeren Sendeintervall hälst somit über eine Woche länger beziehungsweise 7,6 mal so lang wie das andere mit einer Akkuladung durch, dennoch ist der Verbrauch nur marginal geringer als der der Bereichsortungstags. \\
...
Das ist für eine direkte Fernlokalisierung aber nicht notwendig, der RSSI wird auf 
...
Ein Sendevorgang, der nur Schicht 1 (PHY) nutzt hat einen geringeren Energieverbrauch, da er nur senden und nie empfangen muss.
...
\end{verbatim}
Ein gewöhnlicher Probe Request beinhaltet noch zusätzliche Informationen bezüglich seiner technischen Möglichenkeiten, wie etwa unterstützde Standards und Datenraten, da das Tag aber nicht tatsächlich beitreten will kann darauf verzichtet werden. \\
...
Dieser schaltet den Chip und seinen Speicher fast vollständig ab, nach Ablauf der angegebenen Schlafzeit wird Pin 16 mit der negativen Seite der Stromquelle verbunden.
Damit der Chip wieder aufwacht muss Pin 16 mit dem Reset Pin (RST) verbunden werden, da dies zu einem Reset führt initialisiert der Chip neu.
Bei einer Lösung auf einer höheren Schicht würde dies dazu führen, dass das Tag versuchen würde dem Netztwerk erneut beizutreten, hingegen kann bei einer Lösung auf Schicht 2 sofort gesendet und danach wieder geschlafen werden.\\
...
\begin{table}[h]
	...
	\begin{tabular}{p{3cm}|p{2.2cm}|p{2cm}|p{2cm}|p{2cm}}
		SDK & Schlafzustand  & Spannung in V & $\varnothing$ Verbrauch in mA & $\varnothing$ Verbrauch in mW \\
		\hline
		Arduino Core & light\_sleep & 5 & ? & ? \\
		Arduino Core & deep\_sleep & 5 & ? & ? \\
		ESP Open SDK & light\_sleep & 5 & ? & ? \\
