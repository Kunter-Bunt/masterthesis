...
Um die gemessenen 88 m bei 30 km/h zu durchqueren benötigte ein Mitarbeiter circa 10,5 s, bei einem Sendeintervall von 10 s finden demnach zwei Sendevorgänge beim Durchqueren des Einflussbereichs eines APs statt.
Da eine zuverlässige Erkennung von Bereichswechseln gefordert wurde, sollte das Sendeintervall jedoch konservativer gesetzt werden, eine Halbierung auf 5 s ist daher für eine zuverlässige Erkennung sinnvoll.
...
Die mobile Einheit des WiFi-LLS-Systems führt alle 5 s einen \emph{Scan} aus, kodiert die Ergebnisse in XML und versendet sie an den Ortungsdienst \cite{chen2007design}.
...
Die Intervallzeit wird entsprechend der Untersuchung der Reichweite auf 5 s gesetzt.
...
Diese beginnt bei circa einer Sekunde mit dem \emph{Scan}, nachdem sie diesen beendet hat, beginnt sie bei 5 s mit dem \emph{Join}-Vorgang.
...
Ein solches Verhalten ist bei circa 19 s zu erkennen.

Bei 12, 17 und 22 s werden weitere \emph{Scans} ausgeführt, diese stehen im Zentrum der Implementierung, da sie implizit die Position bestimmen.
