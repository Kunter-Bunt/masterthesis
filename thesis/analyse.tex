...
Danach folgt die direkte Fernlokalisierung mit Bluetooth Low Energy und LoRa.
...
Es wird zunächst ein aktiver \emph{Scan} ausgeführt, dazu werden nacheinander auf allen überlappungsfreien Kanälen \emph{Probe Requests} versendet und bei empfangen der \emph{Probe Responses} die MAC-Adresse des antwortenden APs zusammen mit dem RSSI protokolliert.
Nach Beenden des \emph{Scans} werden die Ergebnisse an dem Ortungsdienst übermittelt. 
...
Für die direkte Fernlokalisierung mit IEEE 802.11 wird der \emph{Received Signal Strength Indicator} (RSSI) eingehender Pakete von den mobilen Einheiten an den \emph{Access Points} (APs) bestimmt.
...
\subsection{RADAR}
Für das \emph{RADAR} System von Bahl et al. versenden die mobilen Einheiten UDP-Pakete mit einem sechs Byte langen Identifikator \cite{bahl2000radar}.
...
Beim \emph{Probe Request} handelt es sich um einen \emph{Management Frame}, der auch für den \emph{Scan} verwendet wird.
...
Für die direkte Fernlokalisierung mit Bluetooth Low Energy (BLE) wird der \emph{Received Signal Strength Indicator} (RSSI) eingehender \emph{Advertising}-Pakete von den mobilen Einheiten an den BLE-fähigen Basisstationen bestimmt.
...
Für die direkte Fernlokalisierung mit LoRa wird der \emph{Received Signal Strength Indicator} (RSSI) eingehender Pakete von den mobilen Einheiten an den LoRa-fähigen Basisstationen bestimmt.
